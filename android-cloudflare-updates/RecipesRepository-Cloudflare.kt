...
class RecipesRepository @Inject constructor(
    ...
    /**
     * Load recipes from Cloudflare API instead of local JSON
     */
    private suspend fun loadRecipesFromCloudflare(): RecipeData {
        return try {
            ...
            if (response.isSuccessful) {
                val cloudflareResponse = response.body()
                if (cloudflareResponse?.success == true) {
                    val cloudflareRecipes = cloudflareResponse.data.recipes
                    ...
                    cloudflareRecipes.forEach { cloudflareRecipe ->
                        ...
                        val recipe = Recipe(
                            ...
                            type = cloudflareRecipe.type,
                            name = cloudflareRecipe.name,
                            summary = cloudflareRecipe.summary ?: "",
                            imageURL = cloudflareRecipe.image_url,
                            pinImageURL = cloudflareRecipe.pin_image_url,
                            pinImageRepinID = cloudflareRecipe.pin_image_repin_id ?: "",
                            servings = cloudflareRecipe.servings ?: "1",
                            servingsUnit = cloudflareRecipe.servings_unit ?: "servings",
                            servingsAdvancedEnabled = cloudflareRecipe.servings_advanced_enabled ?: "false",
                            prepTime = cloudflareRecipe.prep_time ?: "0",
                            prepTimeZero = cloudflareRecipe.prep_time_zero ?: "false",
                            cookTime = cloudflareRecipe.cook_time ?: "0",
                            cookTimeZero = cloudflareRecipe.cook_time_zero ?: "false",
                            totalTime = cloudflareRecipe.total_time ?: "0",
                            customTime = cloudflareRecipe.custom_time ?: "",
                            customTimeZero = cloudflareRecipe.custom_time_zero ?: "false",
                            customTimeLabel = cloudflareRecipe.custom_time_label ?: "",
                            videoEmbed = cloudflareRecipe.video_embed ?: "",
                            notes = cloudflareRecipe.notes ?: "",
                        ...
                        // Convert ingredients
                        cloudflareRecipe.ingredients_flat?.forEach { cloudflareIngredient ->
                            val ingredient = Ingredient(
                                id = "${recipe.id}_ingredient_${cloudflareIngredient.uid ?: 0}",
                                recipeId = recipe.id,
                                amount = cloudflareIngredient.amount,
                                unit = cloudflareIngredient.unit,
                                name = cloudflareIngredient.name,
                                notes = cloudflareIngredient.notes,
                                unitID = cloudflareIngredient.unit_id,
                                type = cloudflareIngredient.type
                            )
                            ingredientsList.add(ingredient)
                        }
                        
                        // Convert instructions
                        cloudflareRecipe.instructions_flat?.forEachIndexed { index, cloudflareInstruction ->
                            val instruction = Instruction(
                                id = "${recipe.id}_instruction_${cloudflareInstruction.step_number ?: index}",
                                recipeId = recipe.id,
                                stepNumber = cloudflareInstruction.step_number ?: (index + 1),
                                text = cloudflareInstruction.instruction,
                                imageURL = cloudflareInstruction.image_url
                            )
                        ...
                        // Convert nutrition
                        cloudflareRecipe.nutrition?.let { nutrition ->
                            val nutritionRecord = Nutrition(
                        ...
                        // Convert equipment
                        cloudflareRecipe.equipment?.forEach { cloudflareEquipment ->
                            val equipment = Equipment(
                                id = "${recipe.id}_equipment_${cloudflareEquipment.uid ?: 0}",
                                recipeId = recipe.id,
                                amount = cloudflareEquipment.amount ?: "",
                                name = cloudflareEquipment.name,
                                notes = cloudflareEquipment.notes ?: "",
                                uid = cloudflareEquipment.uid ?: 0
                            )
                    ...
                    println("❌ Cloudflare API returned unsuccessful response")
    ...
    /**
     * Updated refresh method to load from Cloudflare API first, fallback to local JSON
     */
    suspend fun refreshRecipes() {
        try {
            ...
            val recipeData = loadRecipesFromCloudflare()
            println("✅ Loaded ${recipeData.recipes.size} recipes from Cloudflare API")
            
            if (recipeData.recipes.isNotEmpty()) {
                // Insert recipes
                recipeDao.insertAllRecipes(recipeData.recipes)
                println("💾 Inserted ${recipeData.recipes.size} recipes")
                
                // Insert nutrition data
                if (recipeData.nutrition.isNotEmpty()) {
                    nutritionDao.insertAllNutrition(recipeData.nutrition)
                    println("💾 Inserted ${recipeData.nutrition.size} nutrition records")
                }
                
                // Insert ingredients
                if (recipeData.ingredients.isNotEmpty()) {
                    ingredientDao.insertAllIngredients(recipeData.ingredients)
                    println("💾 Inserted ${recipeData.ingredients.size} ingredients")
                }
                
                // Insert instructions
                if (recipeData.instructions.isNotEmpty()) {
                    instructionDao.insertAllInstructions(recipeData.instructions)
                    println("💾 Inserted ${recipeData.instructions.size} instructions")
                }
                
                // Insert tags
                if (recipeData.tags.isNotEmpty()) {
                    tagDao.insertAllTags(recipeData.tags)
                    println("💾 Inserted ${recipeData.tags.size} tags")
                }
                
                // Insert equipment
                if (recipeData.equipment.isNotEmpty()) {
                    equipmentDao.insertAllEquipment(recipeData.equipment)
                    println("💾 Inserted ${recipeData.equipment.size} equipment items")
                }
                
                println("✅ Successfully loaded complete recipe data from Cloudflare API")
            } else {
                println("❌ No recipes found from Cloudflare API")
            }
        } catch (e: Exception) {
            println("💥 Exception loading recipes from Cloudflare API: ${e.message}")
            e.printStackTrace()
        }
    }
...
private fun Recipe.toCloudflareModel(): com.chefpro4home.data.api.CloudflareRecipeModel {
    return com.chefpro4home.data.api.CloudflareRecipeModel(
        ...
        pin_image_repin_id = pinImageRepinID,
        name = name,
        summary = summary,
        servings = servings,
        servings_unit = servingsUnit,
        servings_advanced_enabled = servingsAdvancedEnabled,
        prep_time = prepTime,
        prep_time_zero = prepTimeZero,
        cook_time = cookTime,
        cook_time_zero = cookTimeZero,
        total_time = totalTime,
        custom_time = customTime,
        custom_time_zero = customTimeZero,
        custom_time_label = customTimeLabel,
